...
- **인용문 번역(2026-09-09)**: 2,239개 중 취향 기준으로 128개를 추려 **직접 한국어 번역**을 붙임
  (`ko` 필드). 화면에는 번역이 먼저, 원문이 그 아래 항상 함께. 번역은 공식 번역본이 아니라
  내가 옮긴 것이므로 원문을 반드시 같이 보여줄 것 — 그래야 출처와 대조가 됨.
...
- **배경 러너(2026-09-10 교체)**: `image icon/runner_colors/` 5색(aurora/current/moonlight/
  silver/sunset). 라이트=aurora, 다크=silver. 이전 세트의 "머리카락이 지렁이 같다" 문제가
  입자·연기 표현으로 해결됨. 비율 2.42, **얼굴이 가로 75~91% 지점** → 오프셋은
  `50 + margin + 0.91×width = 102` 을 풀어서 정함(데스크톱 170vw/-102.7vw, 모바일 260vw/-184.6vw).
  순백색이 없어 대비 상한이 0.41~0.51 로 넉넉함(이전 세트는 0.36).
- **엣지 장식 참(2026-09-09)**: `image icon/web-assets/` 의 실사 키링·백참·낙서·자동차를
  화면 가장자리에 배치. 좌우 키링은 100vh, 본문 열을 덜 침범하도록 -14vw/-20vw 로 빼둠.
  **흰 하이라이트가 강해 다크 테마 상한은 0.32** (그 이상이면 흰 하이라이트 위 본문이 AA 미달).
  라이트는 제한 없어 0.5. 1100px 이하에서는 키링·상단참 숨김.
...
- **모바일은 고정(2026-09-09)**: 사용자가 모바일 결과에 만족 → styles.css **맨 끝의
  ...
  모바일에 새지 않음. 모바일을 바꿔야 할 때만 그 블록을 직접 수정.
